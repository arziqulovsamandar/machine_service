...
let OrderItemsService = exports.OrderItemsService = class OrderItemsService {
    ...
    async create(createOrderDto) {
        return this.orderItemModel.create(createOrderDto);
    }
    async update(id, updateOrderDto) {
        const builder = await this.orderItemModel.update(updateOrderDto, {
            where: { id },
            returning: true,
        });
    ...
    async delete(id) {
        const numRowsDeleted = await this.orderItemModel.destroy({
            where: { id },
        });
        if (numRowsDeleted === 0) {
